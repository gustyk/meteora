  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        // Issue 14: in DRY_RUN mode, wallet balance won't change after a
        // simulated deploy, which makes the LLM think the deploy "didn't take"
        // and triggers a retry. Inject an explicit note into the result.
        if (process.env.DRY_RUN === "true") {
          result.dry_run = true;
          result.dry_run_note = "DRY RUN: This was a simulated deployment. The on-chain wallet balance is NOT affected. Trust this result as final — do NOT retry deploy_position.";
        }
